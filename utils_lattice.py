...
def _plot_lattice_graph(
    ...
    displayItemsetLabels=False,  # If True, display itemset labels for length 1 itemsets and maximal
    font_size_ItemsetLabels=10,
    selected_itemsets_plot=None,  # if displayItemsetLabels is True, list of itemsets to display even if not maximal or length 1
):
    ...
    if displayItemsetLabels:
        ...
        order_mapping = {
            v: id_v
            for id_v, v in enumerate(name_mapping)
            if len(v) in [1, max_len] or v in selected_itemsets_plot
        }
        for group_i, a in groups_labels.items():
            for i, itemset in enumerate(a):
                if (
                    len(itemset) not in [1, max_len]
                    and itemset not in selected_itemsets_plot
                ):
                    continue
                ...
                fig.add_annotation(
                    ...
                    ay=get_y_pos(
                        ...
                        (
                            p_ref_y
                            if len(itemset) == 1
                            else (-0.12 if itemset in selected_itemsets_plot else -0.03)
                        ),
                    ),
                    showarrow=True,
                    font=dict(
